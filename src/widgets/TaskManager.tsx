...
export interface TaskColumn {
  ...
  filterable?: boolean;
  sticky?: boolean;
  render?: (value: any, task: Task, index: number) => React.ReactNode; // eslint-disable-line @typescript-eslint/no-explicit-any
}
      ...
      <div className='flex flex-col gap-1'>
        <div className='flex items-center gap-2'>
          {task.priority && (
            ...
            >
              {React.createElement(PRIORITY_STYLES[task.priority].icon, {
                className: 'h-3 w-3',
              })}
            </div>
          )}
          <span className='font-medium text-sm truncate' title={value}>
            {value}
          </span>
        </div>
        {task.description && (
...
const TaskHeader: React.FC<{
  ...
  >
    ...
    <div className='flex items-center gap-2'>
      ...
      {onOpenTaskManager && (
        <Button variant='outline' size='sm' onClick={onOpenTaskManager}>
          📋 Open task manager
        </Button>
      )}
    </div>
...
const TaskFilters: React.FC<{
  ...
  onFilter: (_field: keyof FilterConfig, _value: unknown) => void;
...
const TableRow: React.FC<{
  ...
  setEditingCell: (_cell: { taskId: string; field: keyof Task } | null) => void;
}> = ({
  task,
  columns,
  index,
  ...
  const handleCellEdit = useCallback(
    (taskId: string, field: keyof Task, value: unknown) => {
      if (onTaskUpdate) {
      ...
      if (column.key === 'name' && isEditing && enableInlineEdit) {
        return (
          <Input
            ...
            onBlur={e => handleCellEdit(task.id, column.key, e.target.value)}
            onKeyDown={e => {
              if (e.key === 'Enter') {
                handleCellEdit(
                  task.id,
...
const MemoizedTableRow = React.memo(
  ({
    ...
    data: {
      ...
      editingCell: { taskId: string; field: keyof Task } | null;
      setEditingCell: (
        _cell: { taskId: string; field: keyof Task } | null
      ) => void;
    };
  }) => {
...
export function TaskManager({
  ...
  const handleFilter = useCallback(
    (_field: keyof FilterConfig, _value: unknown) => {
