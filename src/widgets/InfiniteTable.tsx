...
export function InfiniteTable<T = Record<string, unknown>>({
    ...
    useEffect(() => {
        if (onLoadMore && hasNextPage && !loading) {
            ...
            const handleScroll = (event: globalThis.Event) => {
                ...
                if (!target.closest(".schilling-table-container")) return;

                const scrollTop = target.scrollTop;
                const scrollHeight = target.scrollHeight;
                const clientHeight = target.clientHeight;

                // Load more when near the bottom (within 200px) and not already loading
                if (
                    ...
                    // Reset the loading flag after a delay to allow for new data
                    setTimeout(() => {
                        isLoadingMore = false;
                    }, 1000);
                }
            ...
            // Add event listener to all elements that could scroll
            const scrollContainers = document.querySelectorAll(
                ".schilling-table-container, [data-react-window]"
            );
    ...
    }, [onLoadMore, hasNextPage, loading, data.length]);
